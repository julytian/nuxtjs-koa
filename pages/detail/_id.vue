<template>
    <div class="detail container">
        ...
        <div class="detail-meta">
            ...
            <p class="meta meta-tags">标签：
                <span v-for="(tag, index) in article.tags"
                    :key="index">{{ tag.name }}</span>
            </p>
        </div>
        <div class="detail-content">
            <top-preview :content="article.content"
                :options="options"></top-preview>
        </div>
        <div class="detail-admin"
            v-if="isLogin">
            <p class="admin-del">
                <a @click="del(article.id)">删除</a>
            </p>
            <p class="admin-edit">
                <a @click="edit(article.id)">编辑</a>
            </p>
        </div>
        <Tip ref="tip"></Tip>
    </div>
